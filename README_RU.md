...
- **Полная история коммитов** — анализирует каждый коммит, а не только текущее состояние
...
# 2. Настройка (скопировать и отредактировать)
cp .env.example .env
...
# 4. Сканирование репозитория
python -m tokenleak scan https://github.com/user/repo.git

# 5. Сканирование списка репозиториев
...
**Без глобальной установки (рекомендуется для cron):**
```bash
python -m venv venv
...
```bash
# Сканирование конкретного коммита
